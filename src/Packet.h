...
class RequestPacket : public class Packet {
 public:
  RequestPacket(int CUB, int ADRS, int TAG, int LNG, int SLID, Command CMD): type(Type::Request) {
...
class ResponsePacket : public class Packet {
 public:
  ResponsePacket(int CUB, int TAG, int LNG, int SLID, Command CMD): type(Type::Response) {
    header.CUB = Datafield<int>(CUB, cub_bits);
...
class NULLPacket : public class Packet {
 public:
  NULLPacket():type(Type::NULLPacket) {
    payload_flits = 0;
    total_flits = payload_flits + 1;
    flow_control = true;
  }
...
class TRETPacket : public class Packet {
 public:
  TRETPacket(int RTC):type(Type::TRET) {
...
class RETRYPacket : public class Packet {
 public:
  RETRYPacket():type(Type::RETRY) {
    tail.RTC.bits = rtc_bits;
